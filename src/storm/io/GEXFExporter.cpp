...
void GEXFExporter<ValueType>::exportGEXFToStream(
    std::shared_ptr<storm::models::sparse::Model<ValueType>> model, std::ostream &outStream, std::optional<std::vector<std::vector<uint64_t>>> colorVec,
    std::optional<std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>>> additionalAttributesMap, bool allowDefaultColoring,
    bool convertRatesToProbs) {
    ...
    // For nonexistent colors + additionalAttributes, use empty vec/map bc then we don't have to deal with either constantly using .value() of the optionals but
    // also don't have to have eg colors given to be able to give addAttr as we would if we gave the params default values
    ...
    std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> additionalAttributes =
        additionalAttributesMap ? additionalAttributesMap.value() : std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>>();
    ...
    switch (model->getType()) {
        case models::ModelType::Pomdp:
            ...
            exportNonDetGEXFToStream(model->template as<storm::models::sparse::NondeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
        ...
        case models::ModelType::MarkovAutomaton:
            prepMa(model->template as<storm::models::sparse::MarkovAutomaton<ValueType>>(), colors, additionalAttributes, allowDefaultColoring,
                   convertRatesToProbs);
            if (convertRatesToProbs) {
                // We are assuming here (due to the MA constructors (and only those) using the turnRatesToProbabilities function whenever the components give
                // rates) that an MA by default uses a prob matrix
                exportNonDetGEXFToStream(model->template as<storm::models::sparse::NondeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
                                         model->getTransitionMatrix());
            } else {
                exportNonDetGEXFToStream(model->template as<storm::models::sparse::NondeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
                                         model->template as<storm::models::sparse::MarkovAutomaton<ValueType>>()->computeRateMatrix());
            }
            break;
        case models::ModelType::S2pg:
            // ok so the problem with s2pg is that I wanted to write a converter to smg but the class seems incomplete so that it is not clear how basic
            // attributes like numberOfStates are to be understood so idk. do with that what you will. I will keep this case here though in case this gets
            // resolved.
        ...
        case models::ModelType::Dtmc:
            ...
            exportDetGEXFToStream(model->template as<storm::models::sparse::DeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
                                  model->getTransitionMatrix());
            break;
        case models::ModelType::Ctmc:
            ...
            if (convertRatesToProbs) {
                exportDetGEXFToStream(model->template as<storm::models::sparse::DeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
                                      model->template as<storm::models::sparse::Ctmc<ValueType>>()->computeProbabilityMatrix());
            } else {
                exportDetGEXFToStream(model->template as<storm::models::sparse::DeterministicModel<ValueType>>(), outStream, colors, additionalAttributes,
                                      model->getTransitionMatrix());
            }
        ...
        default:
            STORM_LOG_THROW(false, storm::exceptions::NotSupportedException,
                            "GEXF export not supported for this model type: " << model->getType());  // for possible future model types
    }
}

template<typename ValueType>
std::vector<std::string> GEXFExporter<ValueType>::collectStateLabelings(std::shared_ptr<storm::models::sparse::Model<ValueType>> model) {
...
template<typename ValueType>
void GEXFExporter<ValueType>::prepCtmc(std::shared_ptr<storm::models::sparse::Ctmc<ValueType>> ctmc, std::vector<std::vector<uint64_t>> &colors,
                                       std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> &additionalAttributes,
    ...
    if (convertRatesToProbs) {
        if (!additionalAttributes.contains("ExitRate")) {
            ...
            additionalAttributes["ExitRate"] = std::make_pair(
                GEXFAttributeType::GEXF_string,
                std::move(rates));  // using string here bc I don't know if rationals would count as valid double but also don't wanna convert unnecessarily
        } else {
            STORM_LOG_WARN("Attribute \"ExitRate\" is already given and will overwrite the default attribute that is usually generated.");
        }
    }
}

template<typename ValueType>
void GEXFExporter<ValueType>::prepPomdp(std::shared_ptr<storm::models::sparse::Pomdp<ValueType>> pomdp, std::vector<std::vector<uint64_t>> &colors,
                                        std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> &additionalAttributes,
...
template<typename ValueType>
void GEXFExporter<ValueType>::prepMdp(std::shared_ptr<storm::models::sparse::Mdp<ValueType>> mdp, std::vector<std::vector<uint64_t>> &colors,
                                      bool allowDefaultColoring) {
...
template<typename ValueType>
void GEXFExporter<ValueType>::prepDtmc(std::shared_ptr<storm::models::sparse::Dtmc<ValueType>> dtmc, std::vector<std::vector<uint64_t>> &colors,
                                       bool allowDefaultColoring) {
    // This doesn't do much but still putting it in its own function so every model type has a prep function and is thus easily expandable
    if (colors.empty() && allowDefaultColoring) {
        STORM_LOG_INFO("Applying default coloring.");
...
template<typename ValueType>
void GEXFExporter<ValueType>::prepMa(std::shared_ptr<storm::models::sparse::MarkovAutomaton<ValueType>> ma, std::vector<std::vector<uint64_t>> &colors,
                                     std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> &additionalAttributes,
                                     bool allowDefaultColoring, bool convertRatesToProbs) {
    if (colors.empty() && allowDefaultColoring) {
        STORM_LOG_INFO("Applying default coloring.");
...
template<typename ValueType>
void GEXFExporter<ValueType>::prepSmg(std::shared_ptr<storm::models::sparse::Smg<ValueType>> smg, std::vector<std::vector<uint64_t>> &colors,
                                      std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> &additionalAttributes,
                                      bool allowDefaultColoring) {
...
template<typename ValueType>
std::map<storage::PlayerIndex, std::string> GEXFExporter<ValueType>::createPlayersToNameMap(std::shared_ptr<storm::models::sparse::Smg<ValueType>> smg,
                                                                                            const std::map<storage::PlayerIndex, uint64_t> &playerToColorIdx) {
...
template<typename ValueType>
std::map<storage::PlayerIndex, uint64_t> GEXFExporter<ValueType>::createPlayersToColorIdxMap(std::shared_ptr<storm::models::sparse::Smg<ValueType>> smg) {
    // Collect players
...
template<typename ValueType>
void GEXFExporter<ValueType>::exportNonDetGEXFToStream(std::shared_ptr<storm::models::sparse::NondeterministicModel<ValueType>> model, std::ostream &outStream,
                                                       std::vector<std::vector<uint64_t>> colors,
                                                       std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> additionalAttributes,
                                                       storm::storage::SparseMatrix<ValueType> transMatrix) {
    STORM_LOG_ASSERT(colors.empty() || colors.size() == model->getNumberOfStates(),
                     "Color vector has weird size (not zero indicating no coloring, but also not equal to the number of states)!");
    ...
    for (const auto &attribute : additionalAttributes) {
        STORM_LOG_ASSERT(model->getNumberOfStates() == attribute.second.second.size(),
                         "Additional node attribute vector does not have the same number of values as there are states.");
    ...
    for (uint_fast64_t state = 0; state < model->getNumberOfStates(); state++) {
        ...
        outStream << "      <node id=\"" << state << "\" label=\"" << label
                  << "\"><!-- State node -->\n"
                     "        <attvalues>\n"
        ...
        if (useColors) {
            outStream << "        </attvalues>"
                         "        <viz:color r=\""
                      << colors[state][0] << "\" g=\"" << colors[state][1] << "\" b=\"" << colors[state][2]
                      << "\" a=\"1\" />\n"
                         "      </node>\n";
        } else {
            outStream << "        </attvalues>"
        ...
        for (uint_fast64_t action = 0; action < transMatrix.getRowGroupSize(state); action++) {
            if (!singleSelfLoopActions[transMatrix.getRowGroupIndices()[state] + action]) {
                outStream << "      <node id=\"" << state << "a" << action
    ...
    for (uint_fast64_t state = 0; state < model->getNumberOfStates(); state++) {
        for (uint_fast64_t action = 0; action < transMatrix.getRowGroupSize(state); action++) {
            ...
            if (!singleSelfLoopActions[rowIndex]) {
                ...
                outStream << "      <edge source=\"" << state << "\" target=\"" << intermediate << "\" label=\"" << actionLabel
                          << "\"> <!-- State to Intermediate -->\n";
                outStream << "        <viz:color r=\"0\" g=\"0\" b=\"0\"/>\n";
                outStream << "      </edge>\n";
                for (const auto &entry : row) {
                    outStream << "      <edge source=\"" << intermediate << "\" target=\"" << entry.getColumn() << "\" label=\"" << entry.getValue()
                              << "\"> <!-- Intermediate to Successor State -->\n";
                    outStream << "        <viz:color r=\"0\" g=\"0\" b=\"0\"/>\n";
                    outStream << "      </edge>\n";
                }
            } else {
                outStream << "      <edge source=\"" << state << "\" target=\"" << state << "\" label=\"" << actionLabel
                          << "\"> <!-- Almost-Sure Self Loop -->\n";
                outStream << "        <viz:color r=\"0\" g=\"0\" b=\"0\"/>\n";
                outStream << "      </edge>\n";
            }
        }
...
template<typename ValueType>
void GEXFExporter<ValueType>::exportDetGEXFToStream(std::shared_ptr<storm::models::sparse::DeterministicModel<ValueType>> model, std::ostream &outStream,
                                                    std::vector<std::vector<uint64_t>> colors,
                                                    std::map<std::string, std::pair<GEXFAttributeType, std::vector<std::string>>> additionalAttributes,
                                                    storm::storage::SparseMatrix<ValueType> transMatrix) {
    STORM_LOG_ASSERT(colors.empty() || colors.size() == model->getNumberOfStates(),
    ...
    for (uint_fast64_t state = 0; state < model->getNumberOfStates(); state++) {
        ...
        for (const auto &entry : row) {
            outStream << "      <edge source=\"" << state << "\" target=\"" << entry.getColumn() << "\" label=\"" << entry.getValue()
                      << "\"> <!-- State to Successor State -->\n";
            outStream << "        <viz:color r=\"0\" g=\"0\" b=\"0\"/>\n";
            outStream << "      </edge>\n";
        }
    }
...
template<typename ValueType>
std::vector<std::vector<uint64_t>> GEXFExporter<ValueType>::createSmgPlayerColoring(std::shared_ptr<storm::models::sparse::Smg<ValueType>> smg,
                                                                                    const std::map<storage::PlayerIndex, uint64_t> &playerToColorIdx) {
    auto colors = std::vector<std::vector<uint64_t>>();
...
std::vector<std::vector<uint64_t>> GEXFExporter<ValueType>::createBasicColoring(uint64_t numberOfColors) {
    ...
    if (numberOfColors <= 1530) {
        ...
        for (uint64_t state = 0; state < numberOfColors; state++) {
            ...
            colors[state][0] = std::get<0>(rgb) * (255 / (x + 1));  // +1 because x is the number of cuts, thus x +1 is the number of blocks in r direction
...
std::tuple<uint64_t, uint64_t, uint64_t> GEXFExporter<ValueType>::adaptedEuclid(uint64_t i, uint64_t x, uint64_t y) {
    ...
    // (note that the r axis is divided x times, i.e. into x + 1 sections, but including the start and end, we have x + 2 distinct values of interval borders.
    // Thus, r can take x + 2 values which is the size of the interval)
